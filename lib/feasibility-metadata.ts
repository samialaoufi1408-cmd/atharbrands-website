...
export function feasibilityMetadata(locale: Locale, example: boolean | CaseSlug = false): Metadata {
  ...
  return {
    ...
    openGraph: { type: 'website', siteName: 'ATHRBRANDS', title, description, url: `/${locale}${path}`, locale: ar ? 'ar_SA' : 'en_US', images: [study?.hero ?? '/assets/studies/sumra/full-v3.webp'] },
  };
}
