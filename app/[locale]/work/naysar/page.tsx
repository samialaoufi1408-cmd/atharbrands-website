...
import styles from './naysar.module.css';
...
const COPY = {
  ar: {
    ...
    name: 'نَيْسَار', tagline: 'حضورٌ يُحَسّ.', intro: 'من أول نظرة إلى لحظة فتح العبوة؛ لغة واحدة تمنح العطر شخصيته.',
    scope: ['الاستراتيجية والتسمية', 'الهوية البصرية', 'التغليف', 'المتجر والمحتوى'],
    heroAlt: 'مجموعة نيسار: ثلاث زجاجات عطر شفافة بأغطية فضية وعبوات بلون عنابي تحمل الاسم وأرقام المجموعة',
    ...
    naming: 'اسم ابتكاري قصير، بصياغة عربية ولاتينية متقاربة في الإيقاع. يتقدّم الاسم على الواجهة، وتمنح المساحات الهادئة المنتج مساحة ليظهر.',
    ...
    packagingAlt: 'تصور تغليف نيسار: علبة هدايا عنابية مفتوحة، زجاجة عطر، مجموعة عينات وبطاقة عطر',
    ...
    storeCaption: 'تصور تصميم صفحة منتج، ضمن دراسة الهوية.', storeNav: ['المجموعة', 'عن نيسار', 'مجموعة التجربة'],
    ...
    campaignAlt: 'زجاجة نيسار 02 على منصة عنابية مع إضاءة جانبية وقوس شفاف وردي',
    campaignLine: 'حضورٌ يُحَسّ.', campaignSub: 'NAYSAR · 02',
  ...
  en: {
    ...
    name: 'NAYSAR', tagline: 'Presence, felt.', intro: 'From the first glance to the unboxing: one visual language gives the fragrance its character.',
    scope: ['Strategy & naming', 'Visual identity', 'Packaging', 'Digital & content'],
    heroAlt: 'NAYSAR fragrance collection: three clear glass bottles with silver caps and numbered burgundy packaging',
    ...
    naming: 'A short coined name, with Arabic and Latin wordmarks that share a considered rhythm. The name leads on the front face; generous space allows the product to speak.',
    ...
    packagingAlt: 'NAYSAR packaging concept with an open burgundy gift box, perfume bottle, discovery samples and scent card',
    ...
    storeCaption: 'Product-page design concept, presented as part of the identity study.', storeNav: ['Collection', 'About NAYSAR', 'Discovery set'],
    ...
    campaignAlt: 'A NAYSAR 02 bottle on a burgundy plinth with directional light and a translucent rose arc',
    campaignLine: 'Presence, felt.', campaignSub: 'NAYSAR · 02',
...
export function generateMetadata({ params }: Props) {
  return caseMetadata(params.locale, 'naysar');
}

export default function NaysarCase({ params }: Props) {
  const { locale } = params;
  const ar = locale === 'ar';
  const c = COPY[locale];
  return (
    <main className={styles.page} dir={ar ? 'rtl' : 'ltr'}>
      <header className={styles.nav}>
        <Link href={`/${locale}`} className={styles.studio} aria-label={ar ? 'أثر — الرئيسية' : 'ATHR — home'}><Seal variant="full" idSuffix="naysar-nav" /><span>ATHR BRANDS</span></Link>
        <div><Link href={`/${ar ? 'en' : 'ar'}/work/naysar`} lang={ar ? 'en' : 'ar'}>{ar ? 'EN' : 'العربية'}</Link><Link href={`/${locale}#work`}>{c.back}</Link></div>
      </header>

      <section className={styles.intro} aria-labelledby="naysar-title">
        <div className={styles.topline}><span>{c.concept}</span><span>{c.sector}</span></div>
        <div className={styles.titleRow}>
          <div><h1 id="naysar-title" lang={locale}>{c.name}</h1><p className={styles.otherName} lang={ar ? 'en' : 'ar'}>{ar ? 'NAYSAR' : 'نَيْسَار'}</p></div>
      ...
      <figure className={styles.hero}><Image src="/assets/naysar/hero.png" alt={c.heroAlt} width={1536} height={1024} sizes="100vw" priority /></figure>

      <section className={styles.section} aria-labelledby="naysar-brief">
        <div className={styles.sectionHeading}><span className={styles.number}>01</span><p>{c.briefLabel}</p><h2 id="naysar-brief">{c.briefTitle}</h2></div>
      ...
      <section className={styles.identity} aria-labelledby="naysar-identity">
        <div className={styles.identityArt} aria-hidden="true"><NaysarMark className={styles.largeMark} /><div><span className={styles.arabicWordmark}>نَيْسَار</span><span className={styles.wordmark}>NAYSAR</span></div></div>
        <div className={styles.identityCopy}><span className={styles.number}>02</span><p className={styles.eyebrow}>{c.identityLabel}</p><h2 id="naysar-identity">{c.identityTitle}</h2><p>{c.identityBody}</p><p>{c.naming}</p></div>
      </section>
      ...
      <section className={styles.section} id="packaging" aria-labelledby="naysar-packaging"><div className={styles.sectionHeading}><span className={styles.number}>03</span><p>{c.packagingLabel}</p><h2 id="naysar-packaging">{c.packagingTitle}</h2></div><div className={styles.sectionBody}><p>{c.packagingBody}</p></div></section>
      <figure className={styles.wideImage}><Image src="/assets/naysar/packaging.png" alt={c.packagingAlt} width={1536} height={1024} sizes="(max-width: 760px) 100vw, 90vw" /></figure>

      <section className={styles.collection} id="collection" aria-labelledby="naysar-collection"><p className={styles.eyebrow}>{c.collectionLabel}</p><h2 id="naysar-collection">{c.collectionTitle}</h2><p className={styles.collectionIntro}>{c.collectionIntro}</p><div className={styles.editions}>{c.collection.map(item => <article key={item.number}><span className={styles.editionNumber}>{item.number}</span><h3>{item.name}</h3><p>{item.notes}</p><span className={styles.editionRule}/><p>{item.mood}</p></article>)}</div></section>

      <section className={styles.digital} aria-labelledby="naysar-digital"><div className={styles.section}><div className={styles.sectionHeading}><span className={styles.number}>04</span><p>{c.digitalLabel}</p><h2 id="naysar-digital">{c.digitalTitle}</h2></div><div className={styles.sectionBody}><p>{c.digitalBody}</p></div></div>
        <figure className={styles.storeFigure}><div className={styles.storeFrame}>
          <div className={styles.browserBar} aria-hidden="true"><i/><i/><i/><span>NAYSAR · COLLECTION</span></div>
          <div className={styles.storeHeader}><span className={styles.storeBrand}>NAYSAR</span><div>{c.storeNav.map(item => <span key={item}>{item}</span>)}</div></div>
          <div className={styles.storeProduct}><Image src="/assets/naysar/campaign.png" alt={c.campaignAlt} width={1536} height={1024} sizes="(max-width: 760px) 90vw, 45vw" /><div><span className={styles.eyebrow}>{c.storeKicker}</span><h3>{c.storeTitle}</h3><p>{c.storeBody}</p><p className={styles.storeDetail}>{c.storeDetail}</p><a href="#collection">{c.storeLink}<span aria-hidden="true">↗</span></a></div></div>
        </div><figcaption>{c.storeCaption}</figcaption></figure>
      </section>

      <section className={styles.section} aria-labelledby="naysar-campaign"><div className={styles.sectionHeading}><span className={styles.number}>05</span><p>{c.campaignLabel}</p><h2 id="naysar-campaign">{c.campaignTitle}</h2></div><div className={styles.sectionBody}><p>{c.campaignBody}</p></div></section>
      <figure className={styles.campaign}><Image src="/assets/naysar/campaign.png" alt={c.campaignAlt} width={1536} height={1024} sizes="100vw" /><figcaption><span dir="ltr">{c.campaignSub}</span><p>{c.campaignLine}</p><NaysarMark /></figcaption></figure>

      <section className={styles.deliverables} aria-labelledby="naysar-deliverables"><div><p className={styles.eyebrow}>{c.deliveryLabel}</p><h2 id="naysar-deliverables">{c.deliveryTitle}</h2></div><ul>{c.deliverables.map(item => <li key={item}>{item}</li>)}</ul></section>
      <p className={styles.disclosure}>{c.disclosure}</p>
      <ProjectContact locale={locale} project="NAYSAR" />
    </main>
  );
}
