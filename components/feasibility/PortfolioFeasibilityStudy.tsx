  ...
  return <FeasibilityFrame locale={locale} path={`/work/${study.slug}/feasibility`} theme={study.theme} project={{ slug: study.slug, name: study.name[locale] }}>
    <section className={styles.hero}>
      <div><p className={styles.eyebrow}>{t('دراسة جدوى توضيحية · ', 'Illustrative feasibility · ')}{study.sector[locale]}</p>
        ...
        <div className={styles.actions}><a className={`${styles.button} ${styles.buttonFilled}`} href="#model">{t('جرّب الحاسبة', 'Try the calculator')}</a><Link className={styles.button} href={`/${locale}/work/${study.slug}`}>{t('استراتيجية العلامة وهويتها', 'Brand strategy and identity')}</Link>{study.download && <a className={styles.button} href={study.download} download>{t('تحميل الدليل PDF', 'Download PDF guide')}</a>}{study.financialDownload && <a className={styles.button} href={study.financialDownload} download>{t('النموذج المالي Excel', 'Excel financial model')}</a>}</div>
      </div>
    ...
    <div className={styles.notice}><p>{study.scope[locale]}</p><p>{study.evidenceNote?.[locale] ?? t('لم يُجرَ لهذه الدراسة بحث ميداني أو اختبار بيع أو جمع عروض أسعار. جميع الأرقام التجارية افتراضات من إعداد أثر؛ المصادر الرسمية تدعم المنهج أو تحدد جهات التحقق فقط. الإصدار: ', 'No fieldwork, sales test or quotation exercise has been completed for this study. All commercial figures are assumptions prepared by ATHR; official sources support methodology or identify verification authorities only. Version: ')}<bdi>{study.version ?? '2026-09-06'}</bdi>.</p></div>
    <div className={styles.body}>
      ...
      <section className={styles.section} id="budget">{heading(5)}
        <p>{study.budgetNote?.[locale] ?? t('مبالغ افتراضية بالريال السعودي. بنود الإجراءات والتراخيص مخصصات تخطيطية وليست رسومًا منشورة. تعديلات الحاسبة لا تغير الجداول الأساسية التالية.', 'Hypothetical SAR amounts. Administrative and licensing lines are planning allowances, not published fees. Calculator edits do not change these base budget tables.')}</p>
        {development ? <>
      ...
      <section className={styles.section} id="sources">{heading(9)}
        <p>{t('مراجعة المراجع: ', 'References reviewed: ')}<bdi>{study.version ?? '2026-09-06'}</bdi>. {t('تختلف عن تاريخ إصدار كل مرجع. الروابط التالية للمصادر الأصلية؛ أرقام المبيعات والتكاليف الموصوفة بالافتراضية تحتاج تحققًا مستقلًا.', 'This is distinct from each source’s publication date. Links point to original sources; sales and costs labeled as assumptions require independent verification.')}</p>
