...
function SpacePresentation({ locale }: { locale: Locale }) {
  ...
  return <div className={styles.spacePresentation}>
    <div className={styles.demoHeader}><span>RAHB ALDAR</span><span>{ar ? 'استكشف المساحة' : 'Explore the space'}</span></div>
...
function TrackingPresentation({ locale }: { locale: Locale }) {
  ...
  return <div className={styles.trackingPresentation}>
    <div className={styles.trackingBrand}><DarwaqMark/><span>TATABU</span><small>{ar ? 'واجهة تتبّع تصورية' : 'Tracking interface concept'}</small></div>
    <div className={styles.trackingBody}><div><p className={styles.demoEyebrow}>{ar ? 'رحلة الشحنة' : 'The shipment journey'}</p><h3>{ar ? 'اعرف أين وصلت شحنتك.' : 'Know where it stands.'}</h3><p>{ar ? 'تسلسل واضح للحالة الحالية والخطوة التالية.' : 'A clear view of the current state and what comes next.'}</p><div className={styles.demoTag}>{ar ? 'بيانات عرض' : 'Demonstration data'}<span dir="ltr">TT–DEMO</span></div></div>
...
export function BusinessCase({ slug, locale }: { slug: BusinessCaseKey; locale: Locale }) {
  ...
  const caseSlug = project.slug;
  const ar = locale === 'ar';
  const Mark = slug === 'nawsaq' ? NawsaqMark : DarwaqMark;
  const asset = (name: string) => `/assets/${slug}/${name}?v=names-v2`;
  return <main className={`${styles.page} ${styles[project.theme]}`} dir={ar ? 'rtl' : 'ltr'}>
    <header className={styles.nav}>
      <Link className={styles.studio} href={`/${locale}`} aria-label={ar ? 'أثر — الرئيسية' : 'ATHR — home'}><Seal variant="full" idSuffix={`${caseSlug}-nav`} /><span>ATHR BRANDS</span></Link>
      <div><Link href={`/${ar ? 'en' : 'ar'}/work/${caseSlug}`} lang={ar ? 'en' : 'ar'}>{ar ? 'EN' : 'العربية'}</Link><Link href={`/${locale}#work`}>{ar ? 'العودة إلى الأعمال' : 'Back to work'}</Link></div>
    </header>

    <section className={styles.intro} aria-labelledby={`${caseSlug}-title`}>
      <div className={styles.topline}><span>{ar ? 'مشروع تصوري من أثر' : 'An independent ATHR concept'}</span><span>{c.sector}</span></div>
      <div className={styles.titleRow}><div><h1 id={`${caseSlug}-title`}>{c.name}</h1><p className={styles.otherName} lang={ar ? 'en' : 'ar'}>{ar ? project.latinName : project.copy.ar.name}</p></div><div><h2>{c.tagline}</h2><p>{c.intro}</p></div><Mark className={styles.introMark}/></div>
    ...
    <section className={styles.section} aria-labelledby={`${caseSlug}-challenge`}>
      <div className={styles.heading}><span className={styles.number}>01</span><p>{ar ? 'التحدي' : 'The challenge'}</p><h2 id={`${caseSlug}-challenge`}>{c.challenge.title}</h2></div>
    ...
    <section className={styles.identity} aria-labelledby={`${caseSlug}-idea`}>
      ...
      <div className={styles.identityCopy}><span className={styles.number}>02</span><p className={styles.eyebrow}>{ar ? 'الفكرة البصرية' : 'The visual idea'}</p><h2 id={`${caseSlug}-idea`}>{c.idea.title}</h2><p>{c.idea.body}</p></div>
    </section>
    ...
    <section className={styles.section} id="identity-applications" aria-labelledby={`${caseSlug}-applications`}><div className={styles.heading}><span className={styles.number}>03</span><p>{ar ? 'تطبيقات الهوية' : 'Identity applications'}</p><h2 id={`${caseSlug}-applications`}>{c.application.title}</h2></div><div className={styles.body}><p>{c.application.body}</p></div></section>
    ...
    <section className={styles.digital} aria-labelledby={`${caseSlug}-digital`}><div className={styles.section}><div className={styles.heading}><span className={styles.number}>04</span><p>{ar ? 'التجربة الرقمية' : 'The digital experience'}</p><h2 id={`${caseSlug}-digital`}>{c.digital.title}</h2></div><div className={styles.body}><p>{c.digital.body}</p></div></div><figure className={styles.demoFigure}>{slug === 'nawsaq' ? <SpacePresentation locale={locale}/> : <TrackingPresentation locale={locale}/>}<figcaption>{c.digital.caption}</figcaption></figure></section>

    <section className={styles.section} aria-labelledby={`${caseSlug}-environment`}><div className={styles.heading}><span className={styles.number}>05</span><p>{ar ? 'حضور العلامة' : 'The brand in use'}</p><h2 id={`${caseSlug}-environment`}>{c.environment.title}</h2></div><div className={styles.body}><p>{c.environment.body}</p></div></section>
    ...
    <section className={styles.deliverables} aria-labelledby={`${caseSlug}-deliverables`}><div><p className={styles.eyebrow}>{ar ? 'مخرجات الدراسة' : 'Study deliverables'}</p><h2 id={`${caseSlug}-deliverables`}>{ar ? 'نظام جاهز للامتداد.' : 'A system made to extend.'}</h2></div><ul>{c.deliverables.map(item => <li key={item}>{item}</li>)}</ul></section>
